      ...
      <el-dialog title="分类" :visible.sync="categoryDialogVisible" width="30%">
        ...
        <el-form label="left" label-width="80px" :model="category">
          ...
          <el-form-item label="分类描述">
            <el-input v-model="category.cdes"></el-input>
          </el-form-item>
        </el-form>
      ...
      <el-dialog title="提示" :visible.sync="foodDialogVisible" width="40%">
        ...
        <el-form label="left" label-width="80px" :model="food">
          ...
          <el-form-item label="菜品描述">
            <el-input v-model="food.fdes"></el-input>
          </el-form-item>
          <el-form-item label="菜品价格">
            <el-input v-model="food.fprice"></el-input>
          </el-form-item>
          <el-form-item label="菜品库存">
            <el-input v-model="food.famount"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    async getAllFood() {
      const { data: res } = await this.$axios.get("/food/findAllFoods");
      // console.log(data)
    ...
    async deleteNode(node, data) {
      ...
      if (data.type == "category") {
        const { data: res } = await this.$axios.post(
          "food/deleteCategory?cid=" + data.id
        );
      } else {
    ...
    async foodSubmit() {
      ...
      if (this.operation == "add") {
        const { res: data } = await this.$axios.post("food/saveFood", this.food);
