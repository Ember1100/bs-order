	...
	<el-container>
		<el-header>
			...
			<el-dropdown trigger="click">
				<span class="el-dropdown-link">{{ user[0].uname }}<i class="el-icon-arrow-down el-icon--right"></i>
				</span>
		...
		<el-main>
			...
			<el-tabs v-model="activeName">
				<el-tab-pane v-for="(food, index) in foodList" :key="food.cid" :label="food.cname" :name="index + ''">
					<div v-for="item in food.foods" :key="item.fid" class="item">
						<el-image :src="getImgUrl(item.fimg)" @click="clickImg(item)"></el-image>
			...
			<div class="dialog">
				<el-dialog :visible.sync="userInfoDialogVisible" width="50%">
					<el-form label-position="left" label-width="80px" v-model="user">
						<el-form-item label="名称">
							<el-input v-model="user[0].uname" disabled></el-input>
						</el-form-item>
						<el-form-item label="地址">
							<el-input v-model="user[0].uaddress" disabled></el-input>
						</el-form-item>
						<el-form-item label="电话号码">
							<el-input v-model="user[0].upn" disabled=""></el-input>
						</el-form-item>
					</el-form>
				</el-dialog>
				<!-- addFood -->
				<el-dialog :visible.sync="foodDetailDialogVisible" width="40%">
					<el-card class="box-card">
						<div>名称：{{ this.currentFood.fname }}</div>
						<div>价格：{{ this.currentFood.fprice }}</div>
						<div>描述：{{ this.currentFood.fdes }}</div>
						<div>库存量：{{ this.currentFood.famount}}</div>
						<div>
							图片：
							<img :src="getImgUrl(this.currentFood.fimg)" style="width: 250px; heigth: 250px" />
						</div>
						<div>
							数量:
					...
					<span slot="footer" class="dialog-footer">
						<el-button @click="foodDetailDialogVisible = false">取 消</el-button>
                ...
                cartItem.amount
					...
					<span slot="footer" class="dialog-footer">
						...
						<el-button v-if="cart != ''" type="primary" @click="submitCart">提交</el-button>
					</span>
				</el-dialog>
				<!-- orderShow -->
				<el-dialog :visible.sync="orderDialogVisible" width="60%" title="订单详细查询">
				...
				<el-dialog :visible.sync="dialogVisible" width="50%">
					<div>
						<el-steps :space="200" :active="statusTransfer(currentOrderRow.oStatus)"
...
<script>
	...
	export default {
		...
		async created() {
			...
			const {
				data: res
			} = await this.$axios.get("food/findAllFood");
			console.log(res);
		...
		data() {
			return {
				...
				cart: [],
				user: [],
				// user: {uAddress:'',uId: '',uName: '',uPn: '',uPwd: ''},
		...
		methods: {
				...
				switch (id) {
					...
					case "1":
						...
						for (var i = 0; i < this.cart.length; i++) {
							this.totalPrice += this.cart[i].price * this.cart[i].amount;
						}
			...
			addToCart() {
				...
				cartItem.id = this.currentFood.fid;
				cartItem.name = this.currentFood.fname;
				cartItem.price = this.currentFood.fprice;
			...
			edit(row) {
				...
				console.log(this.currentOrderRow);
			...
			async getOrder() {
				const {
					data: res
				} = await this.$axios.get("order/getOrdersByUser", {
					params: {
						uId: this.user[0].uid
					}
				});
